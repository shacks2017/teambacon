...
void OdomTf::odomCallback(const nav_msgs::Odometry& odom_rcvd) {
    ...
    tf::Vector3 estpos;
    estpos = stfEstBaseWrtMap_.getOrigin();
    //get_pose_from_transform(tf::StampedTransform stfEstBaseWrtMap_)
    odom_x_ = estpos[0];
    odom_y_ = estpos[1];
    current_odom_1.pose.pose.position.x = odom_x_;
    current_odom_1.pose.pose.position.y = odom_y_;
    
    //tfQuat = stfEstBaseWrtMap_.getRotation();
    //odom_quat.x = tfQuat.x(); // copy the data from tf-style quaternion to geometry_msgs-style quaternion
    //odom_quat.y = tfQuat.y();
    //odom_quat.z = tfQuat.z();
    //odom_quat.w = tfQuat.w();
    ...
    current_odom_1.pose.pose.orientation = odom_quat_;
}
