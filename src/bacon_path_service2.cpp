...
#include <string>
#include <math.h>
#include <nav_msgs/Odometry.h> //needed for position feedback
using namespace std;
//some tunable constants, global
const double g_move_speed = .10; // set forward speed to this value, e.g. 1m/s
...
geometry_msgs::Pose g_current_pose; // getting (trying to get) from odom 
geometry_msgs::Pose global_pose; // getting (trying to get) from odom 
...
// New function to compute distance and heading from start to goal: 
void get_yaw_and_dist(geometry_msgs::Pose current_pose, geometry_msgs::Pose goal_pose,double &dist, double &heading) {
 double x_dist = goal_pose.position.x - current_pose.position.x;
 double y_dist= goal_pose.position.y - current_pose.position.y;
 double z_dist= goal_pose.position.z - current_pose.position.z; //just for completeness
 dist = sqrt(pow(x_dist, 2) + pow(y_dist, 2) + pow(z_dist, 2)); //Good ol' distance formula
 if (dist < g_dist_tol) { //too small of a motion, so just set the heading from goal heading
   heading = convertPlanarQuat2Phi(goal_pose.orientation); 
    ...
    heading = atan2(y_dist, x_dist); //trig happens! only good in two dimensions, though. 
 }
...

{
    ...
    for (int i=0;i<npts;i++) { //visit each subgoal
        ...
        //g_current_pose = global_pose;
        //FIX THE NEXT LINE, BASED ON get_yaw_and_dist()
        do_move(travel_distance);  // Compute this from subgoal pose
        }
...
{ 
//g_current_pose.orientation = odom.pose.pose.orientation;
//g_current_pose.position = odom.pose.pose.position;
global_pose = odom.pose.pose; //make the pose status global, so main() can use it....
    //ROS_INFO("OdomCallback accomplished"); 
}
...
if(foo==0){
  //initialize components of the twist command global variable
    ...
    //define initial position - in STDR, the robot spawns at (1, 2)
    g_current_pose.position.x = 1.0;
    g_current_pose.position.y = 2.0;
    ...
    foo=1; //dummy variable to ensure this only happens once
        ROS_INFO("InitCallback accomplished");
   } 
