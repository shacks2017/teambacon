...
const double MIN_SAFE_DISTANCE = 1.0; // set alarm if anything is within 1m of the front of robot - increased from 1.0
...
double robot_radius=0.2; //.2m radius for robot0; should probably be picking this up from a subscriber...
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    if(zero_index < 0) {
        ...
        ping_min = zero_index - (int)((PI/2)/angle_increment_);
        ping_max = zero_index + (int)((PI/2)/angle_increment_);
